...
The second dataset comes from the NeuroTycho project~\cite{neurotycho} that designs Brain-Computer \textcolor{red}{съехало почему-то} Interface (BCI)~\cite{millan2010combining,mason2007comprehensive} for information transmitting between brains and electronic devices.
...
\textcolor{red}{The} partial least squares (PLS) algorithm projects the design matrix $\bX$ and the target matrix \textcolor{red}{to} \textcolor{red}{the} latent space $\mathbb{R}^l$ with low dimensionality ($l < r < n$).
...
The disign matrix $\bX$ and the target matrix $\bY$ \textcolor{red}{are} projected into the latent space in the following way:
\begin{align}
\label{eq::PLS_X}
...
\end{align}
\textcolor{red}{where $\bT$} is a matrix of a joint description of the objects and the outcomes in the latent space, and the columns of the matrix $\bT$ are orthogonal; $\bP,\ \bQ$ are transition matrices from the latent space to the original space; $\bE,\ \bF$ are residual matrices.
...
\textcolor{red}{Contours of the distribution covariance matrices} are shown in red.
...
Taking into account the interaction between the matrices $\bX$ and $\bY$ deviates the vectors $\bw_k$ and $\bc_k$ from the principal \textcolor{red}{components - не знаю можно ли писать просто во множественном числе} directions. 
...
To obtain the model predictions and find the model parameters, let multiply the both \textcolor{red}{hand-}sides of the equation~\eqref{eq::PLS_X} by the matrix $\bW$. Since the rows of the residual matrix  $\bE$ are orthogonal to the columns of the matrix $\bW$, we have
\[
	\bX \bW = \bT \bP^{\T} \bW.
\]
...
The algorithm of partial least squares allows to build a \textcolor{red}{vector target variable prediction -> target vector prediction}. 
The latent space contains information about the objects and the responses and dramatically reduces the dimensionsionalies of the input matrices. 
