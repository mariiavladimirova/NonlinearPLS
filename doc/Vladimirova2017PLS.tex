...
\begin{tikzpicture}
    ...
    \node[below of = x, node distance=3em] (x_size) {\footnotesize{$m\times n$}};
    \node[below of = t, node distance=3em] (t_size) {\footnotesize{$m\times l$}};
    \node[below of = p, node distance=1.5em] (p_size) {\footnotesize{$l\times n$}};
    \node[below of = e, node distance=3em] (e_size) {\footnotesize{$m\times n$}};
    ...
    \node[below of = y, node distance=3em] (y_size) {\footnotesize{$m\times r$}};
    \node[below of = u, node distance=3em] (u_size) {\footnotesize{$m\times l$}};
    \node[below of = q, node distance=1.5em] (q_size) {\footnotesize{$l\times r$}};
    \node[below of = f, node distance=3em] (f_size) {\footnotesize{$m\times r$}};
...
\begin{align}
\label{pls_x_y}
    \underset{m \times n}{\bX} &= \underset{m \times l}{\bT} \ \underset{l \times n}{\bP^{\T}} + \underset{m \times n}{\bE},\\
    \underset{m \times r}{\bY} &= \underset{m \times l}{\bU} \ \underset{l \times r}{\bQ^{\T}} + \underset{m \times r}{\bF},
\end{align}
где $\bT,\ \bU$~--- матрицы объектов и ответов в спроектированном пространстве, причём $\bT^{\T} \bT = \bI_{l}$; $\bP,\ \bQ$~--- матрицы перехода из нового пространства в старое; $\bE,\ \bF$~--- матрицы невязок. 
% Размеры векторов в алгоритме можно изобразить следующим образом:




Псевдокод метода регрессии PLS приведен в алгоритме~\ref{PLSR_pseudo}. Алгоритм итеративно на каждом из $l$ шагов вычисляет по одному столбцу $\bt$, $\bu$, $\bp$, $\bq$, из которых путем присоединения формируются матрицы $\bT$, $\bU$, $\bP$, $\bQ$ соответственно. Предполагается, что вектора новых признаков $\bt$ и $\bu$ являются линейными комбинациями столбцов матриц $\bX$ и $\bY$ соответственно.
...
% \end{align*}
\begin{equation*}
    \bw \varpropto \bX^{\T} \bu = \bX^{\T} \bY \bc = \bX^{\T} \bY \bY^{\T} \bt = \bX^{\T} \bY \bY^{\T} \bX \bw,
\end{equation*}
\begin{equation*}
    \bc \varpropto \bY^{\T} \bt = \bY^{\T} \bX \bw = \bY^{\T} \bX \bX^{\T} \bu = \bY^{\T} \bX \bX^{\T} \bY \bc.
\end{equation*}
