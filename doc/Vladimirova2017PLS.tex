...
\centering
\begin{tikzpicture}
    \node (x) [sensor_x]  {$\bX$ \\ \scriptsize{$m\times n$}};
    \node[right of = x, node distance=3em] (x_equal) {=};
    \node[sensor_t, right of = x_equal, node distance=2.5em] (t) {$\bT$ \\ \scriptsize{$m\times l$}};
    \node[right of = t, node distance=2.5em] (x_times) {$\times$};
    \node[sensor_p, right of = x_times, node distance=2.8em] (p) {$\bP^{\T}$ \\ \scriptsize{$l\times n$}};
    \node[right of = p, node distance=2.8em] (x_sum) {+};
    \node[sensor_x, right of = x_sum, node distance=2.8em] (e) {$\bE$ \\ \scriptsize{$m\times n$}};

    \node[below of = x, node distance=7em] (y) [sensor_y]  {$\bY$ \\ \scriptsize{$m\times r$}};
    \node[below of = x_equal, node distance=7em] (y_equal) {=};
    \node[sensor_t, below of = t, node distance=7em] (u) {$\bU$ \\ \scriptsize{$m\times l$}};
    \node[below of = x_times, node distance=7em] (y_times) {$\times$};
    \node[sensor_q, below of = p, node distance=7em] (q) {$\bQ^{\T}$ \\ \scriptsize{$l\times r$}};
    \node[below of = x_sum, node distance=7em] (y_sum) {+};
    \node[sensor_y, below of = e, node distance=7em] (f) {$\bF$ \\ \scriptsize{$m\times r$}};
...
\FOR{$k=1,\dots, l$}
  ...
  \vspace{0.2cm}
  \STATE $\bX_{k+1} := \bX_k - \bt_k \cdot \left(\frac{\bX_k^{\T} \bt_k}{\bt_k^{\T} \bt_k}\right)$
  \vspace{0.2cm}
  \STATE $\bY_{k + 1} := \bY_k - \bt_k \cdot \left(\frac{\bY_k^{\T} \bt_k}{\bt_k^{\T} \bt_k}\right)$ 
\ENDFOR
\end{algorithmic}
\end{algorithm}
\end{center}
