...
В работе решается задача прогнозирования потребления электроэнегрии на основе исторических данных. 
...
Для решения этой задачи используется авторегрессионная модель, то есть предполагается, что значение сигнала в данный момент времени линейно зависит от предыдущих значений этого же сигнала. 
Авторегрессионная модель является неустойчивой из-за наличия мультиколлинеарности в исторических данных. 
Для решения этой проблемы необходимо использовать методы отбора признаков~\cite{Adolph}, в результате чего повышается устойчивость модели без существенного снижения качества прогноза.
...
Выделение признаков происходит итеративно, в порядке уменьшения их влияния на вектор ответов \cite{Ng2013}. Поэтому можно рассматривать только самые значимые комбинации, незначительно потеряв в точности прогноза. 

Методы PLS и PLS регрессии подробно описаны в работах~\cite{Geladi1988, Hoskuldsson1988}. 
...
Нелинейное расширение метода PLSR впервые введено в~\cite{Frank1990}. 
В литературе были разработаны различные модификации PLS. 
Например, функции активации искусственных нейронных сетей используются в методе PLS~\cite{Malthouse1997}. 
...
Пусть $\mathbf{X} = \{\bx_i\}_{i=1}^m \in \mathbb{R}^{m \times n}$ -- матрица признакового описания, $\mathbf{Y}= \{\by_i\}_{i=1}^m \in \mathbb{R}^{m \times r}$ -- матрица ответов. 
Каждая строка $\bx_i$ матрицы $\mathbf{X}$ – локальная история сигнала ($n$ значений сигнала, начиная с момента $i$). 
Каждая строка $\by_i$ матрицы $\mathbf{Y}$ – локальный прогноз сигнала (например, $r$ значений сигнала, начиная с момента $n+1$).
$$
...
Пусть $\mathfrak{D}= \left( \bX, \bY \right)$~--- выборка, $\mathcal{I} = \{1, \ldots, m\}$ -- множество индексов элементов выборки. 
...
Разобъем выборку $\mathfrak{D}$ на обучающую $\mathfrak{D}_{\mathcal{L}}$ и контрольную $\mathfrak{D}_{\mathcal{C}}$, где $\mathcal{L}, \mathcal{C} \in \mathcal{I}$ ($\mathcal{I} = \mathcal{L} \sqcup \mathcal{C}$).
% РИСУНОК
...
Псевдокод метода регрессии PLS приведен в алгоритме~\ref{PLSR_pseudo}. Во внутреннем цикле алгоритма вычисляются скрытые вектора $\bw$ и $\bc$. {\color{red} Сюда про power iteration + ссылка} Обновляя вектора по данным правилам, мы максимизируем ковариацию между векторами $\bt$ и $\bu$
\begin{align*}
    ...
    &= \max_{||\bw|| = 1} \text{cov} ||\bY^{\T} \bX \bw||^2 = \max_{||\bw|| = 1} \bw^{\T} \bX^{\T} \bY \bY^{\T} \bX \bw
\end{align*}
...
\tikzstyle{sensor_x}=[draw, line width=0.4mm, fill=blue!0, text width=4em, text centered, minimum height=5em]
\tikzstyle{sensor_y}=[draw, line width=0.4mm, fill=blue!0, text width=2.7em, text centered, minimum height=5em]
...
\begin{tikzpicture}
    \node (x) [sensor_x]  {$\bX$};
    \path (x.east)+(3.85,0) node (y) [sensor_y] {$\bY$};

    %X
    \path [draw, line width=0.5mm] +(1.2, 1) -- node [right] {$\bt$}
      +(1.2,-1);
    \path [draw, line width=0.5mm] +(0.95, -2) -- node [left of=2] {$\bp^{\T}$}
      +(-0.7, -2); 
    \path [draw, line width=0.5mm] +(0.95, -1.5) -- node [left of=2] {$\bw^{\T}$ }
      +(-0.7, -1.5); 
    %Y
    \path [draw, line width=0.5mm] +(3.9, 1) -- node [left] {$\bu$}
      +(3.9,-1);
    \path [draw, line width=0.5mm] +(4.2, -2) -- node [left of=-1.3] {$\bq^{\T}$}
      +(5.5, -2);
    \path [draw, line width=0.5mm] +(4.2, -1.5) -- node [left of=-1.3] {$\bc^{\T}$}
      +(5.5, -1.5);
\end{tikzpicture}
